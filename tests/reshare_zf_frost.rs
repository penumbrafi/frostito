...
//! FROST(Pallas, BLAKE2b-512)) are reshared with `frostito::reshare` to a
...
fn zf_sign(
 ...
 let params = zf::RandomizedParams::new(pubkeys.verifying_key(), &package, rng).unwrap();
...
fn zf_dkg_then_frostito_reshare_then_zf_sign() {
 ...
 keys::generate_with_dealer(5, 3, keys::IdentifierList::Default, rng).unwrap();
 ...
 let params = zf::RandomizedParams::new(&group_key, &package, rng).unwrap();
 let mut shares = BTreeMap::new();
 for &j in &[1u32, 3, 6] {
 shares.insert(
 id(j),
