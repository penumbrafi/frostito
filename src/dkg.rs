...
//!    ([`Error::InvalidProofOfKnowledge`], [`Error::InvalidSubShare`]);
...
impl<P: CurvePoint> Round1Package<P> {
 ...
 /// # Errors
 ///
 /// [`Error::InvalidProofOfKnowledge`] naming the dealer.
...
impl<P: CurvePoint> Aggregator<P> {
 ...
 /// Aggregator whose dealer set is the one the echo round agreed on.
 ///
...
impl<P: CurvePoint> DkgState<P> {
 ...
 /// recorded: a dealer that cannot prove knowledge of its constant
 /// term never enters the ceremony.
 ///
 /// # Errors
 ///
 /// [`Error::InvalidProofOfKnowledge`] naming the dealer;
 /// [`Error::InvalidIndex`] for an out-of-range index;
 /// [`Error::InvalidCommitment`] on a threshold mismatch;
 /// [`Error::UnexpectedDealer`] if the dealer is disqualified.
 ...
 /// knowledge ([`Error::InvalidProofOfKnowledge`]) or an invalid
 /// sub-share ([`Error::InvalidSubShare`]), both of which carry the
 /// index to pass here.
 ///
 ...
 /// none", which makes undetectable again.
 ///
 ...
 ///
 /// [`Error::InvalidIndex`] for an out-of-range index;
 /// [`Error::DkgAborted`] when fewer than `threshold` dealers remain —
...
/// against cannot be sourced separately, and
...
/// which makes undetectable again. narsild chose the first.
///
...
impl<P: CurvePoint> Complaint<P> {
 ...
 /// with `None` is [`Error::InvalidComplaint`], because there is
 ...
 /// # Errors
 ///
 /// [`Error::InvalidComplaint`] for the wrong ceremony, an index or a
 ...
 /// [`Error::UnexpectedDealer`] when `agreed` holds no commitment for
...
// ============================================================================
// Quorum over non-transferable complaints
// ============================================================================
...
impl ComplaintTally {
 ...
 /// # Errors
 ///
 /// [`Error::InvalidIndex`] for a zero index or a self-accusation — a
...
// ============================================================================
// Echo round over the round-1 set
// ============================================================================
...
/// against, as that commitment was *delivered to this recipient*. That
...
/// commitments, and the epoch is precisely what the proof-of-knowledge
...
impl<P: CurvePoint> AgreedRound1<P> {
 ...
 /// # Errors
 ///
 /// [`Error::EchoMismatch`] — a dealer equivocated, or the broadcast
 ...
 /// # Errors
 ///
 /// [`Error::InsufficientContributions`] if fewer than `expected`
 /// echoes were supplied; [`Error::EchoMismatch`] if any disagrees.
...
impl<P: CurvePoint> DkgState<P> {
 ...
 /// # Errors
 ///
 /// [`Error::InsufficientContributions`] unless round 1 is complete —
...
mod tests {
 ...
 /// The DKG property the OSST assertions used to stand in for: any `t` of
 /// the dealt shares interpolate to the secret behind `group_key`.
 ...
 // ── justified, ceremony-bound, signed complaints ─────────────────
 ...
 /// The residual leaves: a fabricated complaint against a wholly honest
 ...
 // ── dealer equivocation is caught by the echo round ──────────────
 ...
 /// the finding itself: a dealer that commits to one polynomial toward
 ...
 /// and passes its Feldman check and its commitment-digest check. The
 ...
 // knowledge verifies for each, so does not see this
 ...
 // any t of the dealt shares interpolate to the secret behind group_key
 ...
 // shares 1, 3, 5, 7 — non-consecutive indices, at threshold
