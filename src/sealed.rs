...
//!
//! This is finding D-1 (Critical) of SECURITY-REVIEW-2026-09.md, observed in
...
//!   nothing (D-2); here the pair is bound together and to the sender.
//!
//! # What it does not do
//!
...
pub const X25519_DERIVE_INFO: &[u8] = b"osst/sealed/x25519/v1";

/// Domain tag for the ceremony transcript.
pub const TRANSCRIPT_DOMAIN: &[u8] = b"osst/sealed/transcript/v1";
...
impl SealedRoster {
    ...
    /// # Errors
    ///
    /// [`OsstError::EmptyContributions`] for an empty set,
    /// [`OsstError::InvalidIndex`] for index 0, and
    /// [`OsstError::DuplicateIndex`] for a repeated identifier.
    pub fn new(
        ...
        for w in sorted.windows(2) {
            if w[0].0 == w[1].0 {
                return Err(OsstError::DuplicateIndex(w[0].0));
            }
        }
        Ok(Self {
            participants: sorted,
    ...
    pub fn public_key(&self, index: u32) -> Result<&[u8; 32], OsstError> {
        self.participants
            .iter()
            .find(|(i, _)| *i == index)
            .map(|(_, k)| k)
            .ok_or(OsstError::UnknownParticipant(index))
    }
...
/// # Errors
///
/// [`OsstError::UnknownParticipant`] if the recipient is not on the roster;
/// [`OsstError::InvalidIndex`] if the sub-share does not address that
/// recipient or does not come from `commitment`'s dealer;
/// [`OsstError::SealedOpenFailed`] if Noise refuses the key material.
pub fn seal_subshare<P: OsstPoint>(
    dealer_x25519_secret: &[u8; 32],
    roster: &SealedRoster,
    round: u8,
    subshare: &SubShare<P::Scalar>,
    commitment: &DealerCommitment<P>,
) -> Result<SealedSubShare, OsstError> {
    ...
    .ok_or(OsstError::SealedOpenFailed(subshare.dealer_index))?;
...
/// # Errors
///
/// [`OsstError::SealedOpenFailed`] — deliberately uninformative — when the
/// package does not open or its indices disagree with its envelope;
/// [`OsstError::InvalidSubShare`] when it opens but the commitment digest or
/// the Feldman check fails; [`OsstError::UnknownParticipant`] for a dealer the
/// roster does not name.
pub fn open_subshare<P: OsstPoint>(
    ...
    // D-2: the sub-share and the commitment it is checked against must be the
    // pair the dealer sent, not two values an attacker sourced separately.
    if subshare.dealer_index != commitment.dealer_index
        || plaintext[40..] != commitment_digest(commitment)
    {
        return Err(OsstError::InvalidSubShare(sealed.dealer_index));
    }
...
pub fn seal_round2<P: OsstPoint>(
    dealer: &crate::dkg::Dealer<P>,
    dealer_x25519_secret: &[u8; 32],
    roster: &SealedRoster,
    round: u8,
) -> Result<Vec<SealedSubShare>, OsstError> {
...
mod tests {
    ...
    fn the_roster_is_order_independent_and_unambiguous() {
        ...
        assert_eq!(
            ...
            Err(OsstError::DuplicateIndex(1))
        );
        assert_eq!(
            SealedRoster::new(&[(0u32, [0u8; 32])], SESSION),
            Err(OsstError::InvalidIndex)
        );
        assert_eq!(SealedRoster::new(&[], SESSION), Err(OsstError::EmptyContributions));
    }

    /// The property the module exists for (D-1): the secret scalar is not on
    ...
    /// Sender binding (D-2): the `ss` mix puts the sender's static key in the
    ...
    /// D-2, the swapped pair: verifying a sub-share against a commitment the
    ...
    fn a_sealed_round2_completes_the_dkg() {
        ...
        let group_key = {
            let mut k = <Point as OsstPoint>::identity();
            for d in &dealers {
        ...
        let lag = crate::compute_lagrange_coefficients::<
            <Point as OsstPoint>::Scalar,
        >(&[1, 2, 3])
        .unwrap();
        let mut secret = <<Point as OsstPoint>::Scalar as OsstScalar>::zero();
        for (i, s) in shares.iter().enumerate() {
            secret = secret.add(&lag[i].mul(s));
        }
        assert_eq!(<Point as OsstPoint>::generator().mul_scalar(&secret), group_key);
    }
}
