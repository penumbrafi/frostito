...
pub fn compute_lagrange_coefficients<S: CurveScalar>(indices: &[u32]) -> Result<Vec<S>, Error> {
 let k = indices.len();
 if k == 0 {
 ...
 return Err(Error::DuplicateIndex(sorted[i]));
 }
 }
...
mod tests {
 ...
 assert!(matches!(result, Err(Error::DuplicateIndex(2))));
 }
 ...
 assert!(matches!(result, Err(Error::EmptyContributions)));
 }
